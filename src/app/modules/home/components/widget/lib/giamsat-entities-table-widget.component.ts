...
export class GiamsatEntitiesTableWidgetComponent extends PageComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    onAction: () => {
      ...
      getListAllErrorObservable.pipe(timeout(120000)).subscribe((dataError: any[]) => {
        getListErrInCommonObservable.pipe(timeout(120000)).subscribe((dataListErrInCommon: any[]) => {
          ...
          getReportDataObservable.pipe(timeout(300000)).subscribe((data: any[]) => {
            ...
            data.forEach(item => {
              try {
                ...
                console.log("WO_ERR_ARR",WO_ERR_ARR);
                ...
                console.log("timeBatDauSX ",timeBatDauSX);
                item.ThoiGianBatDauSX = timeBatDauSX.toString() == "01/01/1970 08:00:00" ? "" : timeBatDauSX;
                console.log("item.ThoiGianBatDauSX ",item.ThoiGianBatDauSX);
            ...
            data.forEach(item => {
              let { createdTime, his_list_stage, list_stage, running_Time, start_Time, status, stop_Time, stopping_Time,
                cycle_Time, computing_Productivity, line_Id, first_Time_Start_Line, oldstatus, planning_Time, quota, resume_Time, speed_02, serial_stage, ...restPro } = item;
  ...
  exportToExcel(arr: any[], sheetName, fileName, arrStageDetail: any[], arrListErrStage: any[], arrListSerial: any[]) {
    let Heading = [['Mã Planning WO', 'Tên WO-LOT', 'Mã đơn hàng', 'Đơn vị Sản xuất', 'Tổ', 'SAP WO', 'LotNumber', 'Sản lượng kế hoạch',
      'Mã hàng hóa', 'Tên hàng hóa', 'Dây chuyền sản xuất', 'Sản lượng đầu vào', 'Sản lượng hoàn thành', 'Tỉ lệ hoàn thành',
      'Cycle_Time trung bình', 'Thời gian bắt đầu sản xuất', 'Thời gian chạy', 'Thời gian dừng', 'Ngày tiếp nhận', 'Kiểu dây chuyền',
      'OEE', 'PPM', 'Tổng lỗi', 'Độ tin cậy', 'Năng suất thực tính', 'Trạng thái']];
    //Had to create a new workbook and then add the header
    const wb = XLSX.utils.book_new();
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet([]);
    XLSX.utils.sheet_add_aoa(ws, Heading);

    // ws['P2'].t = 'd'; // 'd' là kiểu dữ liệu ngày tháng
    // ws['P2'].z = 'mm/dd/yyyy hh:mm:ss';
    ...
    XLSX.utils.sheet_add_json(ws, arr, {
      ...
      header: ['planning_Code', 'name', 'po_Id', 'branch', 'group', 'sap_Wo', 'lot_Number', 'number_Of_Planning', 'product_Code',
        ...
        'ThoiGianChay', 'ThoiGianDung', 'NgayTiepNhan', 'line_Type', 'oee', 'ppm', 'TongLoi', 'DoTinCay', 'NangSuatThucTinh', 'TrangThai'],
      origin: 'A2',
      skipHeader: true,
    });
    ...
    let HeadingStageDetail = [['Mã Planning WO', 'Tên WO-LOT', 'Mã đơn hàng', 'Đơn vị Sản xuất', 'Tổ', 'SAP WO', 'LotNumber',
    ...
    let HeadingListErrStage = [['Mã Planning WO', 'Tên WO-LOT', 'Mã đơn hàng', 'Đơn vị Sản xuất', 'Tổ', 'SAP WO', 'LotNumber',
    ...
    XLSX.utils.sheet_add_json(wsListSerial, arrListSerial, {
      header: ['planning_Code', 'name', 'po_Id', 'sap_Wo', 'lot_Number', 'stage', 'serialBoard', 'serial', 'lastUpdateTs'],
      origin: 'A2',
      skipHeader: true,
    });
